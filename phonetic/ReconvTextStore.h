#ifndef __RECONV_TEXT_STORE_H_INCLUDED__
#define __RECONV_TEXT_STORE_H_INCLUDED__
#include <olectl.h>
#include <msctf.h>
#include <msaatext.h>
...
//! 変換候補リスト取得用のテキストストア
class ReconvTextStore : public ITextStoreACP
{
public:
	//! オブジェクトの生成
	static HRESULT CreateInstance(ReconvTextStore*& o_text_store_cp);
	//! デストラクタ
	virtual ~ReconvTextStore();
	...
	//==========================================================================
	// ReconvTextStore では実装しないメソッド。E_NOTIMPL を返す。 
	//==========================================================================
...
private:
	//! コピーコンストラクタは使用禁止とする。
	ReconvTextStore(const ReconvTextStore&);
	//! 代入演算子は使用禁止とする。
	const ReconvTextStore& operator=(const ReconvTextStore&);
...
#endif	// #ifndef __RECONV_TEXT_STORE_H_INCLUDED__
